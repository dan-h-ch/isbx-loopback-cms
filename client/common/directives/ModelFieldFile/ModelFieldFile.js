angular
  .module("dashboard.directives.ModelFieldFile", ["dashboard.services.GeneralModel"])
  ...
  .directive("modelFieldFileEdit", function(
    $compile,
    $document,
    $window,
    GeneralModelService,
    ...
    return {
      restrict: "E",
      template:
        '<button class="btn btn-default select-file" ng-hide="disabled" >{{ selectFileButtonText }}</button> \
      ...
      link: function(scope, element, attrs) {
        ...
        $translate(translationBtnKeys).then(function(translated) {
