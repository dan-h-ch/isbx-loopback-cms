angular
  .module("dashboard.services.FileUpload", ["dashboard.Config", "dashboard.Utils", "ngCookies", "angularFileUpload"])
  ...
  .service("FileUploadService", function($cookies, $q, $upload, Config, Utils) {
    ...
    this.getFileUploadData = function(credentials, fileType) {
      return {
        ...
        "Content-Type": fileType != "" ? fileType : "application/octet-stream", // content type of the file (NotEmpty),
    ...
    this.uploadFile = function(file, path) {
      ...
      self.getS3Credentials(path, fileType ? fileType : "", isRegistrySurvey).then(
        function(credentials) {
          $upload
            .upload({
              url: credentials.uploadUrl, //S3 upload url including bucket name,
              method: "POST",
              data: self.getFileUploadData(credentials, fileType),
              file: file
            })
            ...
            .success(function(data) {
              ...
              try {
                var locationPath = xmldoc.evaluate(
    ...
    this.uploadImages = function(imageFiles) {
      ...
      //Get S3 credentials from Server
      self.getS3Credentials(uploadFilePath, file.type, false).then(
        function(credentials) {
          $upload
            .upload({
              url: credentials.uploadUrl, //S3 upload url including bucket name,
